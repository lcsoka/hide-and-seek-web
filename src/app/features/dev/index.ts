...
/** Entry point for the dev cockpit: open an existing session or spin up a test one. */
@Component({
  selector: 'app-dev-index',
  imports: [FormsModule],
  template: `
    <main class="mx-auto w-full max-w-md space-y-6 p-4 sm:p-6">
      <h1 class="text-center text-2xl font-bold">Dev cockpit</h1>
      ...
      <section class="space-y-3 rounded-xl border border-gray-200 bg-white p-4 dark:border-gray-700 dark:bg-gray-900">
        <h2 class="font-semibold">New test session</h2>
...
export class DevIndex {
  ...
  async open(): Promise<void> {
    await this.router.navigate(['/dev/s', this.sessionId.trim()]);
  }
  ...
  async createTest(): Promise<void> {
    ...
    try {
      ...
      const session = await this.api.createSession({ city: 'budapest', game_size: 'medium' });
      await this.router.navigate(['/dev/s', session.id]);
    } catch (e: any) {
