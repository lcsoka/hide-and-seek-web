...
import { OsmDeductionService } from './osm-deduction.service';
...
/** Build the deduction service backed by the given fake OverpassService. */
function serviceWith(features: Feature<Point>[]): OsmDeductionService {
  TestBed.resetTestingModule();
  TestBed.configureTestingModule({
    providers: [{ provide: OverpassService, useValue: overpassWith(features) }],
  });
  return TestBed.inject(OsmDeductionService);
}
...
describe('osm region cuts', () => {
  ...
  it('measuring "closer" keeps inside the circle around the seeker\'s nearest feature', async () => {
    const r = await serviceWith(airports).region(question('measuring', { lat: 47.5, lng: 19.0, feature: 'airport' }, { answer: 'closer' }));
  ...
  it('measuring "further" keeps outside that circle', async () => {
    const r = await serviceWith(airports).region(question('measuring', { lat: 47.5, lng: 19.0, feature: 'airport' }, { answer: 'further' }));
  ...
  it('matching "yes" keeps the seeker\'s nearest-feature cell, "no" drops it', async () => {
    const yes = await serviceWith(airports).region(question('matching', { lat: 47.5, lng: 19.0, feature: 'airport' }, { answer: 'yes' }));
    ...
    const no = await serviceWith(airports).region(question('matching', { lat: 47.5, lng: 19.0, feature: 'airport' }, { answer: 'no' }));
  ...
  it('tentacles "out_of_range" removes the radius circle around the seeker', async () => {
    const r = await serviceWith([]).region(question('tentacles', { lat: 47.5, lng: 19.0, feature: 'zoo', radius_m: 2000 }, { answer: 'out_of_range' }));
    expect(r!.within).toBe(false);
  ...
  it('tentacles "in_range" keeps the nearest place\'s cell within the radius', async () => {
    ...
    const r = await serviceWith(zoos).region(question('tentacles', { lat: 47.5, lng: 19.0, feature: 'zoo', radius_m: 3000 }, { answer: 'in_range', feature_name: 'North Zoo' }));
    expect(r).not.toBeNull();
    expect(r!.within).toBe(true);
