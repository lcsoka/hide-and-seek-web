...
export class UnitsService {
  ...
  readonly radarPresets: Record<Units, DistancePreset[]> = {
    metric: [
  ...
  readonly thermoPresets: Record<Units, DistancePreset[]> = {
    metric: [
      { label: '500 m', meters: 500 },
      { label: '1 km', meters: 1000 },
  ...
  unitsOf(config: Record<string, unknown> | undefined | null): Units {
    return config?.['units'] === 'imperial' ? 'imperial' : 'metric';
  }

  formatDistance(meters: number, units: Units): string {
    if (units === 'imperial') {
